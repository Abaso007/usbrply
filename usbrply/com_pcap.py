...
class PcapGen(object):
    ...
    def packnum(self):
        ...
        if self.arg_packet_numbers:
            postfix = f"{self.submit.packet_number}/{self.pktn_str()}"
        else:
            postfix = "N/A"
        self.pcomment(f"Generated from packet {postfix}")
    ...
    def gen_data(self):
        ...
        while True:
            ...
            if self.verbose:
                print(f"gen_data: {len(self.jbuff)} new JSON entries")

            # Pop packets
            yield from self.jbuff
            self.jbuff = []
    ...
    def run(self):
        ...
        self.comment_source()
        self.gcomment(f"cmd: {' '.join(sys.argv)}")
        ...
        self.printv(f"parsing from range {self.min_packet} to {self.max_packet}")
