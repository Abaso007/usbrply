...
def gett(d):
    if "t" in d["submit"] or "t_urb" in d["submit"]:
        return d["submit"]["t"]
    else:
        raise Exception(
...
''',
    ...
    def packet_number_str(self, d):
        if self.packet_numbers:
            return f'packet {d["submit"]["packn"]}/{d["complete"]["packn"]}'
        else:
    ...
    def parse_data(self, d):
        ...
        if d["type"] == "controlRead":
            ...
            data = "buff" if d["wLength"] else "NULL"
            indented(
            ...
            comment(f'IRP_INFO(): func {d["submit"]["urb"]["usb_func_str"]}')
        elif d["type"] == "abortPipe":
            comment("ABORT_PIPE()")
        elif self.verbose:
            print(f'LibusbPyPrinter WARNING: dropping {d["type"]}')
