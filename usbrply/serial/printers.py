    ...
    def next_json(self, j, prefix=None):
        if j['type'] == 'read':
            ...
            if self.ascii:
                ...
                indented(f"  {binascii.hexlify(prefix)}")
        elif j['type'] == 'write':
            interface = j['interface']
            data = j['data']
            n = len(data) / 2
            ...
            indented(
                f"i{j['interface']} SET_DATA: parity {j['parity']}, stop bits {j['stopbits']}, break {j['breakon']}"
            )
        else:
            indented(f"i{j['interface']} {j['type']}: {j}")
